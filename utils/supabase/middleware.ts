...
export async function updateSession(request: NextRequest) {
    ...
    const isApiOrStatic = pathname.startsWith("/_next") || pathname.startsWith("/api") || pathname.includes(".")

    if (isApiOrStatic) return supabaseResponse

    if (!user && !isPublicRoute) {
        // Auth required but user not connected
