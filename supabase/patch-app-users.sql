-- ═══════════════════════════════════════════════════════════════════════════════
-- OptiStock AI - Fix unique constraints for multi-tenancy
-- Run this in: Supabase Dashboard → SQL Editor → New Query
-- Run this AFTER migration-multi-tenant.sql
-- ═══════════════════════════════════════════════════════════════════════════════

-- ─── 1. Fix categories: name must be unique PER USER, not globally ───────────
-- Drop the old global unique constraint
ALTER TABLE categories DROP CONSTRAINT IF EXISTS categories_name_key;

-- Add composite unique: same name is OK as long as user_id differs
ALTER TABLE categories ADD CONSTRAINT categories_name_user_unique UNIQUE (name, user_id);

-- ─── 2. Fix products: sku must be unique PER USER, not globally ──────────────
ALTER TABLE products DROP CONSTRAINT IF EXISTS products_sku_key;
ALTER TABLE products ADD CONSTRAINT products_sku_user_unique UNIQUE (sku, user_id);

-- ─── 3. Fix app_users: add owner_id column for per-user team members ─────────
ALTER TABLE app_users ADD COLUMN IF NOT EXISTS owner_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;

DROP POLICY IF EXISTS "Allow all for now" ON app_users;
DROP POLICY IF EXISTS "Users manage own app_users" ON app_users;
CREATE POLICY "Users manage own app_users" ON app_users
  FOR ALL USING (auth.uid() = owner_id) WITH CHECK (auth.uid() = owner_id);

-- ─── 4. Re-create the trigger with improved SKU generation (uses full UUID) ──
...
DECLARE
  ...
  uid UUID := NEW.id;
  sku_suffix TEXT := REPLACE(uid::TEXT, '-', '');
BEGIN
  ...
  -- Categories (name uniqueness is now per user_id, so this is safe)
  ...
  -- Products (sku uses full UUID hex to guarantee global uniqueness)
  INSERT INTO products (id, name, sku, category_id, cost_price, selling_price, lead_time_days, perishable, user_id) VALUES
    (prod_macbook, 'MacBook Pro 14"',      'ELEC-001-' || LEFT(sku_suffix,8), cat_electronics, 1499.00, 1999.00, 7,  FALSE, uid),
    (prod_airpods, 'AirPods Pro',          'ELEC-002-' || LEFT(sku_suffix,8), cat_electronics,  189.00,  249.00, 5,  FALSE, uid),
    (prod_usbhub,  'USB-C Hub',            'ELEC-003-' || LEFT(sku_suffix,8), cat_electronics,   29.00,   49.00, 3,  FALSE, uid),
    (prod_milk,    'Organic Milk 1L',      'FOOD-001-' || LEFT(sku_suffix,8), cat_food,           3.50,    5.99, 2,  TRUE,  uid),
    (prod_shoes,   'Nike Air Max',         'SPRT-001-' || LEFT(sku_suffix,8), cat_sports,        89.00,  149.00, 14, FALSE, uid),
    (prod_tv,      'Samsung TV 65"',       'ELEC-004-' || LEFT(sku_suffix,8), cat_electronics,  799.00, 1299.00, 10, FALSE, uid),
    (prod_cases,   'Phone Cases Assorted', 'ACCS-001-' || LEFT(sku_suffix,8), cat_accessories,    5.00,   19.99, 7,  FALSE, uid),
    (prod_mouse,   'Wireless Mouse',       'ELEC-005-' || LEFT(sku_suffix,8), cat_electronics,   19.00,   39.99, 4,  FALSE, uid);
